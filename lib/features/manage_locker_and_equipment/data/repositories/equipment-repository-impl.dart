...
class EquipmentRepositoryImpl implements EquipmentRepository {
  ...
  @override
  Future<Either<RestFailure, List<TypeEquipment>>> getType() async {
    final token = await _authenticationRepository.getFirebaseUser!.getIdToken();
    final result = await _equipmentRestApi.getType(token);
    return result.fold((l) => Left(l), (r) {
      final data = r['data'] as List;
      final types = data.map<TypeEquipment>((e) {
        print(e);
        return TypeEquipment.fromJson(e as Map<String, dynamic>);
      });
